    ...
    return (
      <View>
        <FlatList
          ...
          ItemSeparatorComponent={() => { return(<View style={{borderBottomWidth: 1,BorderColor:"pink"}}/>)
          }}
          renderItem={({item}) => {
            return(
              <TouchableOpacity onPress={() => this.onPress(item.fullname)} style={{flexDirection: 'row',marginTop: 10,}}>
                <View>
                  <Image
                ...
                <View style={{justifyContent:"center",backgroundColor:"pink",width:"90%",alignItems: 'center',}}>
                  <Text>{item.Name}</Text>
                  <Text>{item.ID}</Text>
